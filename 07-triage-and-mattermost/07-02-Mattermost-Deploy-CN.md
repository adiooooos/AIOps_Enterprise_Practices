...
| # | 维度 | 说明 |
| --- | --- | --- |
| ① | **方式 1** | 在线安装：`podman-compose` + `compose.yml` |
| ② | **方式 2** | 离线安装：`docker save` / `scp` / `podman load` |
| ③ | **端口** | `8065/tcp`（firewalld） |
| ④ | **Webhook** | `AllowedUntrustedInternalConnections` 放行 `10.0.0.0/8` |
