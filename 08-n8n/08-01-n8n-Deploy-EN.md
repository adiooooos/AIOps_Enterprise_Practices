# n8n Package Preparation and Deployment
...
| --- | --- | --- |
| ① | **Method 1** | Online deploy: `podman pull` + `podman run` |
| ② | **Method 2** | Offline deploy: `docker save` / `scp` / `docker load` |
| ③ | **Port** | `8080` (`N8N_PORT`) |
| ④ | **First login** | Create or specify admin account |

---

## 1. Online Deployment

### 1.1 Pull Registry Image
...
After the container starts, n8n is reachable via hostname or IP. On first access, create or specify an admin account and password.
(The account is typically an admin email, e.g. admin@example.com in this example.)

---

## 2. Offline Deployment

### 2.1 Download and Transfer on Internet-Connected Host
