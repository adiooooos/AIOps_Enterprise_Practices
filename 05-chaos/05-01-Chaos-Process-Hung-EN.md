...
| --- | --- | --- |
| ① | **Use case** | Use Case 1: Process Hung fault simulation |
| ② | **Target node** | Chaos Server · app directory `/Fault_Simulation` |
| ③ | **Simulation app** | `cpu_stress_webapp.py` — simulates web app peak CPU load to trigger Process Hung–related alert scenarios |
| ④ | **Simulate fault** | Start `python3 cpu_stress_webapp.py --duration 600` · End `Ctrl+C` |

## Overview: Use Case 1 — Process Hung Fault Simulation

We deploy a Python application on the Chaos Server—a business application that simulates webapp peak load causing a Process Hung scenario. The source code of `cpu_stress_webapp.py` is shown below.
...
## cpu_stress_webapp.py Source Code
...
> The `--duration` value (seconds) can be set as needed.
