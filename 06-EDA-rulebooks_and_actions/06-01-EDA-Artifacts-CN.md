...
| # | 维度 | 说明 |
| --- | --- | --- |
| ① | Rulebook | Use Case 1（Process Hung）与 Use Case 2（Network Error） |
| ② | Action Playbook | `01_EDA_action_linuxperformancealerts_UI.yml` · `02_EDA_action_network_down_UI_OK.yml` |
| ③ | AAP UI 配置 | Rulebook Activation · Action Job Template（`webhook_payload`） |
| ④ | 其余编排 | Use Case 1/2 自愈 Playbook · Use Case 3 xsos 诊断 Playbook |
...
Use Case 1：Process Hung 故障仿真 的rulebook如下：
  ...
  rules:
    - name: "Linux perf alerts — match HighSystemCpuUsage or CriticalSystemCpuUsage"
      ...
      actions:
        ...
        #           并指定 Inventory（例如 rhel9 inventory）和 Project，
        #           Playbook 选用本目录下的
        #             01_EDA_action_linuxperformancealerts_UI.yml
...
Use Case 2：Network Error 故障仿真 的rulebook如下：
...
#
# 关键设计点（与 01_eda_rule_linuxperformancealerts_ui.yml 一致）：
#   1. 不使用 run_module / run_playbook，避免 AAP UI 下
#      "action which needs inventory to be defined" 错误。
#   2. 所有副作用（写文件、调用 n8n、采集网络数据、尝试启用网卡）
#      都迁移到 job_template 对应的 Playbook 中执行。
#   3. condition 仅匹配 NetworkInterfaceDown 告警。
#   4. 通过 job_args.extra_vars.webhook_payload 把整份 event.payload
#      原样透传给被触发的 Job Template。
#   5. limit 用 event.payload.commonLabels.instance_name，把动作限定到
#      真正故障的目标主机（例如 chaos.example.com）。
# =============================================================================
...
## 三、EDA Rulebook 的 Action Playbook 开发

### 3.1 Use Case 1 对应 Action Playbook
...
#
#   1. 把 EDA Rulebook 通过 job_args.extra_vars 传过来的 webhook_payload
#      （即 Alertmanager 原始 Webhook Payload）落地为
#         /tmp/<inventory_hostname>-<alertname>-alert_info.txt
#      —— 这一步取代了 CMD 版本 rulebook 里的 run_module: copy 动作。
#   2. 定位高负载进程：top + pidstat 快照，挑出 CPU>90% 的进程。
#   3. 函数级性能剖析：对目标 PID 触发一次 perf record。
#   4. 把性能数据 + alert_info 文件传输到 Triage / Diagnose Server。
#   5. 在 Diagnose Server 上执行 perf report，合并诊断文件，生成
#      issues_latest.txt 最终报告。
#   6. 【最后一个 Play】调用 n8n_webhook_url，激活 n8n 工作流。
#      —— 这一步取代了 CMD 版本 rulebook 里的 run_module: uri 动作。
#
# 改造要点（对比 CMD 版本 01_EDA_action_linuxperformancealerts_OK.yml）：
#   - vars 增加 webhook_payload 默认值（防止手工执行该 Playbook 时未传参报错）。
#   - vars 派生出 alert_name / alert_instance / alert_info_file 等便捷字段。
#   - 新增 "Phase 0: 从 webhook_payload 生成 alert_info 文件" 阶段，
#     代替原 rulebook 在 EDA 容器里执行的 copy 动作。
#   - n8n_webhook_url 调用迁移到独立的最后一个 play 中执行。
# =============================================================================
...
- name: "Linux Performance Analysis for High CPU Usage"
  ...
  tasks:
    ...
    # -------------------------------------------------
    # 需求 4：CMD 版本里 /tmp/<host>-*-alert_info.txt 由 rulebook 的
    # run_module: copy 写入；改造为 AAP UI 后，rulebook 不再能写远程文件，
    # 因此改由 Playbook 在故障节点上根据 webhook_payload 直接生成该文件。
    # ===========================================
...
### 3.2 Use Case 2 对应 Action Playbook
...
#
#   Phase 0: 把 EDA Rulebook 通过 job_args.extra_vars 传过来的 webhook_payload
#            （即 Alertmanager 原始 Webhook Payload）落地为
#               /tmp/<inventory_hostname>-<alertname>-alert_info.txt
#   Phase 1: 识别故障网卡（优先 webhook_payload.commonLabels.device，
#            未提供时扫描 ip link 找出 state DOWN 的物理网卡）
#   Phase 2: 收集网络诊断数据：
#              - ip a / ip link / ip route / ip -s link show <dev>
#              - nmcli general / device / connection / device show <dev>
#              - ethtool <dev> / ethtool -i <dev>
#              - /sys/class/net/<dev>/{carrier,operstate}
#              - journalctl -u NetworkManager / dmesg | grep <dev>
#            统一写入 net_diag.txt
#   Phase 3: 诊断服务器连接和目录准备
#   Phase 4: 全部诊断文件 + alert_info.txt 推送到 Triage Server
#   Phase 5: 合并所有诊断文件为 issues_latest.txt（在 Triage Server 上）
#   最后一个 Play：调用 n8n_webhook_url 激活 n8n 工作流
#
# 设计与 01_EDA_action_linuxperformancealerts_UI_OK.yml 完全对齐：
#   - 不做任何修复
#   - vars 给出 webhook_payload 默认值（手工执行不报错）
#   - vars 派生 alert_name / alert_device / alert_info_file 等
#   - n8n_webhook_url 调用迁移到独立最后一个 play
# =============================================================================
...
## 四、EDA Rulebook 的 Action Job Templates 配置
...
## 五、其余 AIOPS 相关 Playbook 与 JTs 开发和配置

### 5.1 Use Case 1 自愈 Playbook
...
### 5.3 Use Case 3：Unexpected Outage 全科诊断 Playbook
